...
function Set-FStarConfig { param ([switch]$TestOnly, [switch]$PassThru, [switch]$Silent, [Cache]$Cache = $null)
    $meta = Get-Meta

    #--- Guard ---
    if (-not (($null -ne $Cache) -and ($true -eq $Cache.TestRootConfigResult))) {
        if (-not (Test-TestSimpleOutput (Set-RootConfig -TestOnly -Silent))) {
            Write-Debug "$($meta.RootConfig) is not desired."
            return
        }
    }
    #---|
